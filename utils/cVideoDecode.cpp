...
public:
  ...
  virtual void setYuv420 (int width, int height, uint8_t* buffer, int stride) {}
...
//}}}
//{{{
class cFramePlanarBgra : public cVideoDecode::cFrame {
public:
...
public:
  ...
  // table lookup convert, bug in first pix pos stride != width
    ...
    uint8_t* dstPtr1 = dstPtr + (width * 4);
    const int dstRowInc = (width + width - stride) * 4;
