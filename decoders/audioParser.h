...
public:
  cAudioParser() {}
  cAudioParser (uint8_t* framePtr, int len) : frame(framePtr), frameLen(len) {}
  //{{{
  bool parseFrame (uint8_t* stream, uint8_t* streamLast) {
...
// return streamAudioFrameType
  ...
  uint8_t* frame;
  int frameLen;
  eAudioFrameType audioFrameType;
  int skip;
  int frameSampleRate;
  while (parseAudioFrame (stream, streamLast, frame, frameLen, audioFrameType, skip, frameSampleRate)) {
    if (audioFrameType == eId3Tag)
      tags++;
    else {
